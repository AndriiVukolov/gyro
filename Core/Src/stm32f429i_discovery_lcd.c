...
uint8_t BSP_LCD_Init(void)
{
    ...
    */
    ...
    //    PeriphClkInitStruct.PLLSAI.PLLSAIR = 2; //4
...
uint32_t BSP_LCD_ReadPixel(uint16_t Xpos, uint16_t Ypos)
{
    ...
    if (LtdcHandler.LayerCfg[ActiveLayer].PixelFormat ==
        ...
        /* Read data value from SDRAM memory */
        ret = (*(__IO uint32_t *)(LtdcHandler.LayerCfg[ActiveLayer]
                                          .FBStartAdress +
                                  (4 * (Ypos * BSP_LCD_GetXSize() + Xpos))) &
               0x00FFFFFF);
    } else if ((LtdcHandler.LayerCfg[ActiveLayer].PixelFormat ==
...
void BSP_LCD_DisplayChar(uint16_t Xpos, uint16_t Ypos, uint8_t Ascii)
{
    DrawChar(Xpos,
             ...
             &DrawProp[ActiveLayer].pFont->table
                      [(Ascii - ' ') * DrawProp[ActiveLayer].pFont->Height *
                             ...
                             Text_AlignModeTypdef mode)
{
    ...
    case CENTER_MODE: {
        refcolumn =
                X + ((xsize - size) * DrawProp[ActiveLayer].pFont->Width) / 2;
        break;
    }
    case LEFT_MODE: {
        refcolumn = X;
...
void BSP_LCD_DrawCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
    ...
    while (curx <= cury) {
        BSP_LCD_DrawPixel(
                (Xpos + curx), (Ypos - cury), DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(
                (Xpos - curx), (Ypos - cury), DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(
                (Xpos + cury), (Ypos - curx), DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(
                (Xpos - cury), (Ypos - curx), DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(
                (Xpos + curx), (Ypos + cury), DrawProp[ActiveLayer].TextColor);
...
void BSP_LCD_DrawEllipse(int Xpos, int Ypos, int XRadius, int YRadius)
{
    ...
    do {
        BSP_LCD_DrawPixel((Xpos - (uint16_t)(x / k)),
                          (Ypos + y),
                          DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel((Xpos + (uint16_t)(x / k)),
                          (Ypos + y),
...
void BSP_LCD_DrawBitmap(uint32_t X, uint32_t Y, uint8_t *pBmp)
{
    ...
    for (index = 0; index < height; index++) {
        ...
        ConvertLineToARGB8888(
                (uint32_t *)pBmp, (uint32_t *)address, width, inputcolormode);
...
void BSP_LCD_FillEllipse(int Xpos, int Ypos, int XRadius, int YRadius)
{
    ...
    do {
        BSP_LCD_DrawHLine((Xpos - (uint16_t)(x / K)),
                          (Ypos + y),
                          (2 * (uint16_t)(x / K) + 1));
        BSP_LCD_DrawHLine((Xpos - (uint16_t)(x / K)),
                          (Ypos - y),
    ...
    for (i = 0; i < height; i++) {
        ...
        for (j = 0; j < width; j++) {
            if (line & (1 << (width - j + offset - 1))) {
                BSP_LCD_DrawPixel(
                        (Xpos + j), Ypos, DrawProp[ActiveLayer].TextColor);
            } else {
                BSP_LCD_DrawPixel(
                        (Xpos + j), Ypos, DrawProp[ActiveLayer].BackColor);
            }
        }
        Ypos++;
    }
                       ...
                       uint32_t ColorIndex)
{
    ...
    if (HAL_DMA2D_Init(&Dma2dHandler) == HAL_OK) {
        if (HAL_DMA2D_ConfigLayer(&Dma2dHandler, LayerIndex) == HAL_OK) {
            if (HAL_DMA2D_Start(&Dma2dHandler,
                                ColorIndex,
                                (uint32_t)pDst,
                                xSize,
                                ySize) == HAL_OK) {
